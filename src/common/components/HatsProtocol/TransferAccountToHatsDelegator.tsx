...
const TransferAccountToHatsDelegatorSteps: TransferAccountToHatsDelegatorStepType[] = [
  ...
  {
    state: TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.PENDING_PREPARE_TO_RECEIVE_CONFIRMATION,
    title: 'Pending confirmation',
    description: 'Waiting for confirmation that the contract is ready to receive your Farcaster account',
    idx: 2,
  },
  {
  ...
  {
    state: TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.PENDING_SIGNATURE_CONFIRMATION,
    title: 'Pending confirmation',
  ...
  },
  {
    ...
    title: 'Connected',
    description: 'Signature generated, please execute the onchain transfer of your Farcaster account',
    idx: 5,
  },
  {
    state: TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.PENDING_ONCHAIN_CONFIRMATION,
    title: '',
      ...
      case TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.PENDING_SIGNATURE_CONFIRMATION:
        return (
          <div className="flex flex-col">
            <div>
              <p>
                Transferring your Farcaster account to delegator contract{' '}
                <a
                  className="underline"
                  href={`https://optimistic.etherscan.io/address/${toAddress}`}
                  target="_blank"
              ...
              <p className="mt-4">
                This requires a signature and one onchain transaction to transfer the account.
                <br />
                The admin wallet address must sign the transaction. The wallet that owns the Farcaster account must make
                the onchain transaction.
              </p>
            </div>
          </div>
        )
      case TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.CONFIRMED:
        return (
          <div className="flex flex-col text-foreground">
            <p>Your account was successfully transferred 🥳 It is now a shared account!</p>
            <Button
              variant="outline"
    ...
    <div className="flex flex-col w-2/3 space-y-4">
      ...
      {errorMessage && (
        <div className="flex flex-start items-center mt-2">
          <p className="text-wrap break-all	text-sm text-red-500">Error: {errorMessage}</p>
        </div>
      )}
      {(!isConnected || !address) && (
        <div className="w-2/3">
          {step.state === TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.EXECUTE_ONCHAIN &&
            !connectedAddressOwnsFarcasterAccount && (
              <Label>You must connect a wallet that owns the {user.display_name} Farcaster account</Label>
            )}
        </div>
      )}
      ...
      <Button
        ...
        disabled={
          ...
          step.state === TRANSFER_ACCOUNT_TO_HATS_DELEGATOR_STEPS.PENDING_ONCHAIN_CONFIRMATION
        }
        onClick={() => onClick()}
      >
        {getButtonLabel()}
